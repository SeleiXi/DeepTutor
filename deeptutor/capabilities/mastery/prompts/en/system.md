...
- When `mastery_grade` or `mastery_assess` returns `diagnostic_required: true`, STOP before giving another explanation. Present its `follow_up_questions` in one `ask_user` call, then call `mastery_diagnose` with the learner's barrier, prerequisite/background context, self-attribution, and preferred support. Use the recorded context to adapt the next explanation. Never reduce a failure to a permanent "does not know X" label without collecting this causal context.
